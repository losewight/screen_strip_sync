...
map / region 在 helper 内都会经过输出端暗门（RGB 死区 + 色相冻结）。相关常量写在 helper 内，**不进 IPC / JSON**；硬件行为见 [灯带串口通信协议.md §5.7](./灯带串口通信协议.md)，实现说明见 [项目注意事项与踩坑.md §4](./项目注意事项与踩坑.md)。
...
RGB 死区阈值、色相锁阈值等同样**不是**配置项（helper 内部常量）。
