...
DispatchResult dispatch_line(const char *line, HANDLE *serial, SOCKET client) {
  ...
  // D1 调试：死区旋钮只进引擎原子量，不写 JSON / 不推 cfg
  if (strncmp(line, "set deadzone_ton ", 17) == 0) {
    ...
    if (end == p || *end != '\0') {
      printf("bad set deadzone_ton: [%s]\n", p);
    } else {
      engine_set_deadzone_ton((int)v);
      printf("cmd=set deadzone_ton\n");
    }
    return DispatchResult::Continue;
  }
  if (strncmp(line, "set deadzone_toff ", 18) == 0) {
    ...
    if (end == p || *end != '\0') {
      printf("bad set deadzone_toff: [%s]\n", p);
    } else {
      engine_set_deadzone_toff((int)v);
      printf("cmd=set deadzone_toff\n");
    }
    return DispatchResult::Continue;
  }
  if (strncmp(line, "set deadzone_n ", 15) == 0) {
    ...
    if (end == p || *end != '\0') {
      printf("bad set deadzone_n: [%s]\n", p);
    } else {
      engine_set_deadzone_off_frames((int)v);
      printf("cmd=set deadzone_n\n");
    }
    return DispatchResult::Continue;
  }
  if (strncmp(line, "set deadzone_enable ", 20) == 0) {
    ...
    if (end == p || *end != '\0' || (v != 0 && v != 1)) {
      printf("bad set deadzone_enable: [%s]\n", p);
    } else {
      engine_set_deadzone_enable(v != 0);
      printf("cmd=set deadzone_enable\n");
    }
    return DispatchResult::Continue;
  }
