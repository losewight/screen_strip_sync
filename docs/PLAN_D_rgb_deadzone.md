...
### 灯带显示模型（2026-09-26 确认）

硬件 **忽略 RGB 里的亮度信息**：非零颜色一律以 BRT=`63` 的统一亮度显示，等效于「按 max 通道归一化到 255 后再点灯」。软件在暗尾只能控制两件事：

1. **色相**（通道比例）
2. **亮 / 灭**（是否全零）

因此暗部「假渐变」不仅看不见亮度爬坡，还会把 **uint8 取整后的粗档色相** 和 **黑位偏色 / 噪声** 以正常亮度放大成「极光」跳色。与用户 α 无关——α=1 也会出现。
...
| 慢变黑极光 | 暗尾取整粗档 + 偏色噪声被硬件按满亮度显示；旧 `last_nz` 的 `scale_to_min1` 还会把橙压成黄 |

**算法目标**：承认死区不可见 —— **跳过死区做过渡；回死区干净灭灯**。  
不是「把暗部抹得更平滑」，也不是「永远留一点底光」。暗尾保色相（输出前归一化）+ 低亮冻结色相（freeze），直到暗门真灭。
...
ON  时：显示色走用户 EMA+0.49；输出前按 max→255 归一化保色相；
       L≤kHueLockOff 冻结最后可靠色相；取整为 0 则 last_nz 色相兜底（不再 scale_to_min1）
```
...
| `kHueLockOn` / `kHueLockOff` | 32 / 12 | 低亮色相锁：可靠区更新 locked；锁区冻结色相至暗门灭（见 §6.2） |
...
## 6.2 慢变黑极光（2026-09-26 · A + freeze）

### 根因（硬件色相模型）

灯带非零色按 max 归一化后以 BRT=`63` 统一亮度显示。暗尾三处色相失真被放大：

1. **uint8 取整粗档**：max=1 时只剩 7 种纯色；变暗过程中色相在档间跳
2. **暗部色度不可靠**：黑位偏色、压缩噪声、`near_black` 悬崖
3. **旧 `last_nz` + `scale_to_min1`**：橙 `(220,120,30)` 被压成 `010100` 黄

### 修法

| 层 | 做法 | 常量 |
|----|------|------|
| A 必做 | 组出前 `normalize(max→255)` 保色相；`last_nz` 改存色相；删 `scale_to_min1` | — |
| B 选定 **freeze** | `L=max(hold)` 迟滞：≥`kHueLockOn` 更新可靠色相；≤`kHueLockOff` 冻结至暗门灭 | On=32 / Off=12 |

未选：cut（更早灭，α=1 噪声仍有跳色）、neutral（灭前闪白）。

0.49 保持仍作用在归一化前的 `hold`。α=1 时 DXGI 本身是 uint8，采样端精度丢失由 freeze 兜住。

### 离线对照（`fade_aurora_diag`，硬件归一化后色相跳变 >15°）

| 场景 | baseline | A | A+cut | **A+freeze** | A+neutral |
|------|----------|---|-------|--------------|-----------|
| 干净淡出 α=1 hue_jumps | 6 | 0 | 0 | **0** | 0 |
| 偏蓝黑位+噪声 α=1 | 250 | 270 | 24 | **0** | 1 |
| 偏蓝黑位+噪声 α=0.3 | 58 | 30 | 0 | **0** | 0 |
| near_black 悬崖 α=1 | 73 | 0 | 0 | **0** | 0 |

A 单独在偏色噪声下会把噪色放大；freeze 色相最稳。构建：`cmake --build cpp_core/build --config Release --target fade_aurora_diag`。

### 实机验收

彩色画面慢淡出，α=1 与开平滑各一次：暗尾不应段间彩虹跳色；暗场不闪；硬切黑仍约 100ms 灭；暗→亮仍约 50～100ms。
